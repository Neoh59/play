...
You can add additional configuration files to the Play configuration by specifying a file name in @application.conf@ as the value of a configuration option with <code>@include.</code> or <code>@include-absolutepath.</code> at the start of the key.
...
The <code>@include-absolutepath.</code> works in the same way although it doesn't search the included file in the standard @conf@ directory but you have to set a complete path to the file you want to include.

bc. @include-absolutepath.mime = /etc/play/config/mime-types.conf

By the way, you can set this value as a system property. Like this you are able to specify a configuration file at runtime (it could be useful if you deploy your application as a war)

bc. @include-absolutepath.mime = ${play.config.mime-types}
play run -Dplay.config.mime-types=/etc/play/config/mime-types.conf
